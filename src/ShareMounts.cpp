...
public:
    ...
    void OnLogin(Player* player) override
    {
        ...
        std::set<uint32> allianceFactionIds = { 1, 3, 4, 115, 1629 };
        std::set<uint32> hordeFactionIds = { 2, 5, 6, 116, 1610 };
        ...
        std::set<uint32> ignoreSpells = { 54197 };
        ...
        do {
            ...
            uint32 mountCreatureId = spellInfo->Effects[EFFECT_0].MiscValue;

            if (spellInfo && !isSpellMount(spellInfo) && !player->HasSpell(spellId) && ignoreSpells.count(spellId) == 0) {

                if (ignoreSpells.count(spellId) == 0) {
                    if (allianceMountIds.count(spellId) > 0 && allianceFactionIds.count(playerFaction) > 0) {
                        player->learnSpell(spellId);
                    }
                    else if (hordeMountIds.count(spellId) > 0 && hordeFactionIds.count(playerFaction) > 0) {
                        player->learnSpell(spellId);
                    }
